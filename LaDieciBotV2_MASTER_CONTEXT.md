...
- Backend live after WA-UX-ACK-OPERATOR-GATED-01 deploy on 2026-05-27: `c38ca94441e02b58f9141ccf47c7b3e3ae119ba7` (`c38ca94 fix ack whatsapp operator gated flows`). Railway deployment ID `d723619f-d42c-4386-b316-fdceca348628`, boot `2026-05-27T08:08:37.999Z`. `/version` reports commit `c38ca94`, branch `main`; `/health` OK; `/status` backend/database/ordini green (global yellow only for whatsappInbound/Processed right after restart with no recent messages, not blocking).
- Backend live after WA-CANCEL-GUARD-01 deploy on 2026-05-27: `2a525af8f5edfa3955cfc99aecae79ec815fb07a` (`2a525af fix guard whatsapp cancel intent`). Railway deployment ID `0881a161-a32b-4325-99d1-7cbc0f106746`, boot `2026-05-27T11:31:16.304Z`. `/version` reports commit `2a525af`, branch `main`; `/health` OK; `/status` backend/database/ordini green.
- Backend local HEAD, `origin/main`, and Railway production were verified aligned to `e14abd6` on 2026-05-26, to `f8fe69f` on 2026-05-27, to `c38ca94` on 2026-05-27, and to `2a525af` on 2026-05-27.
...
- `origin/backup/wa-ux-ack-operator-gated-01-2026-05-27` → `c38ca94441e02b58f9141ccf47c7b3e3ae119ba7` (in repo `ladieci_bot`, not `LaDieciBotV2`).
- `origin/backup/wa-cancel-guard-01-2026-05-27` → `2a525af8f5edfa3955cfc99aecae79ec815fb07a` (in repo `ladieci_bot`, not `LaDieciBotV2`).
...
- `WA-CANCEL-GUARD-01` closed and live on 2026-05-27. Backend repo `ladieci_bot`, commit `2a525af8f5edfa3955cfc99aecae79ec815fb07a` (`2a525af fix guard whatsapp cancel intent`), backup branch `backup/wa-cancel-guard-01-2026-05-27`, Railway deployment ID `0881a161-a32b-4325-99d1-7cbc0f106746`, boot `2026-05-27T11:31:16.304Z`. Scope: `src/agents/orchestrator.js` only (+27 lines). No frontend, no DB/schema/migration, no env/config. Adds a `cancel_guard` placed BEFORE the Flusso 2B/2C/1 and BEFORE the Flusso 3 (Claude-libre) branches. When the inbound message contains an explicit cancellation verb (`cancela`, `cancelar`, `cancelaci[oó]n`, `cancelad[oa]`, `anula`, `anular`, `anulad[oa]`, `olv[ií]dalo`, `d[eé]jalo`, `dejalo` — note: `quita/quitar` deliberately NOT included to avoid breaking legitimate Flusso 2B `correccion` modifications like "quita la cerveza") AND the customer already has an active order or a `confermata` conv (`statoOrd.haOrdine` OR `conv.stato_ordine === "confermata"`), the bot does NOT cancel the order automatically. The order in `ordenes` remains `POR_CONFIRMAR`, `ordine_ref` is preserved in `wa_msgs` (which is marked `IN_TRATTAMENTO` via in-place update), the client receives the honest ack "Recibido. Lo pasamos al equipo para revisarlo y te respondemos enseguida. 🙏", and the orchestrator returns `flusso: "cancel_guard", motivo: "cancel_intent_operator_review"`. Whitelist (`isWhitelist`) bypasses the guard so operator/owner can still test cancellations. Bug closed: previously the IA classified "Cancela el pedido por favor." as `tipo="domanda"`, the message went into Flusso 3 (`generaRisposta` Claude libre), and Claude hallucinated "Hemos cancelado tu pedido #006" while the order stayed `POR_CONFIRMAR` in DB (CONVQUAL batch T8, classified BLOCKING for Wednesday). Production validation 2026-05-27 (live commit `2a525af`): T1 cancel-natural (`34699001301`, "Cancela el pedido por favor.") PASS — order `#001` untouched, `ordine_ref="#001"` preserved, ack present, no "hemos cancelado"; T2 cancel-variant (`34699001302`, "Olvídalo, ya no lo quiero.") PASS — order `#002` untouched, `ordine_ref="#002"` preserved, ack present; T3 regression normal question (`34699001303`, "Tenéis pizza carbonara?") PASS — guard does not fire, Flusso 3 answers normally with real menu item ("La Joya (Carbonara) 15€"), no order, no false `IN_TRATTAMENTO`. DB cleanup completed post-test; `storico`/`archivio_conv`/`manual_giros`/`geo_cache` untouched.

- `WA-UX-ACK-OPERATOR-GATED-01` closed and live on 2026-05-27. Backend repo `ladieci_bot`, commit `c38ca94441e02b58f9141ccf47c7b3e3ae119ba7` (`c38ca94 fix ack whatsapp operator gated flows`), backup branch `backup/wa-ux-ack-operator-gated-01-2026-05-27`, Railway deployment ID `d723619f-d42c-4386-b316-fdceca348628`. Scope: `src/agents/orchestrator.js` (+20/-5) and `src/utils/helpers.js` (+6/-6) only. No frontend, no DB/schema/migration, no env/config. Adds short, honest client acknowledgements in the operator-gated branches that previously returned silently: (a) delivery without address → "¿Adónde te lo llevamos? Pásanos la dirección completa (calle, número y piso si lo hay) y lo revisamos enseguida. 🍕"; (b) `fuera_de_zona` → "Gracias. Vamos a comprobar la zona de entrega con el equipo y te respondemos enseguida. 🛵"; (c) `modifica_complessa` (cancellations and complex modifications) → "Recibido. Lo pasamos al equipo para revisarlo y te respondemos enseguida. 🙏". Same ack also applied in the Flusso 1 `solo_ora` variants. Normal-order intro phrases (`introConferma` in `helpers.js`) rewritten to remove promissory wording ("el pizzaiolo ya tiene los ojos en tu pedido", "horno ya está en marcha", "horno no para esta noche") and replaced with honest formulas ("Hemos recibido tu pedido…", "Lo pasamos al equipo para confirmarlo"), reflecting the actual `POR_CONFIRMAR` state. DB logic unchanged: no orders/clients are created where they weren't created before; `ordine_ref` is preserved on cancellation/modification. Production validation 2026-05-27: T1 no-address (`34699001101`) PASS, T2 out-zone Aguadulce (`34699001102`) PASS, T3 cancel after order (`34699001103`) PASS — order untouched, `ordine_ref` preserved, ack present; T4 normal order intro sanity (`34699001104`) PASS — honest intro, no promissory phrases, no false positives on guards. DB cleanup completed post-test; storico/archivio_conv/manual_giros/geo_cache untouched. Residual risks noted: (1) `profile.name` still echoed verbatim in confirmation replies (cosmetic, post-Wednesday); (2) operator must actually respond to handed-off cases or customer stays in `in_attesa`; (3) rare `fuera_de_zona` branch inside `solo_ora` still creates `POR_CONFIRMAR` order (preexisting behaviour, intentionally not modified to avoid touching DB logic pre-Wednesday); (4) upsell beer/dessert still present in normal-order replies, not blocking.

...
- `WA-CANCEL-GUARD-01` is live on Railway production as of 2026-05-27. Backend repo `ladieci_bot`, commit `2a525af8f5edfa3955cfc99aecae79ec815fb07a` (`2a525af fix guard whatsapp cancel intent`, +27 lines in `src/agents/orchestrator.js` only). Backup branch `backup/wa-cancel-guard-01-2026-05-27`; Railway deployment ID `0881a161-a32b-4325-99d1-7cbc0f106746`, boot `2026-05-27T11:31:16.304Z`; `/version` reports commit `2a525af`. No frontend/DB/schema/migration/env touched. Intercepts explicit cancellation verbs (`cancela`, `anula`, `olvídalo`, `déjalo`, ...) before Flusso 3 (Claude-libre), so the bot can no longer hallucinate "Hemos cancelado" while the order stays `POR_CONFIRMAR` in DB. Order untouched, `ordine_ref` preserved, `wa_msgs=IN_TRATTAMENTO`, honest ack to the customer. T1/T2/T3 post-fix tests on production all PASS; CONVQUAL T8 BLOCKING bug closed. Live-fix chain for Wednesday: `f8fe69f` (allergy) → `c38ca94` (UX ack) → `2a525af` (cancel guard).
- `WA-UX-ACK-OPERATOR-GATED-01` is live on Railway production as of 2026-05-27. Backend repo `ladieci_bot`, commit `c38ca94441e02b58f9141ccf47c7b3e3ae119ba7` (`c38ca94 fix ack whatsapp operator gated flows`); two files modified: `src/agents/orchestrator.js` (+20/-5) and `src/utils/helpers.js` (+6/-6). Backup branch `backup/wa-ux-ack-operator-gated-01-2026-05-27`; Railway deployment ID `d723619f-d42c-4386-b316-fdceca348628`, boot `2026-05-27T08:08:37.999Z`. Adds honest, short customer acks in operator-gated branches (no address, out of zone, complex modify/cancel) and rewrites the normal-order intro to remove promissory phrasing while preserving `POR_CONFIRMAR` semantics. No frontend/DB/schema/migration/env touched. T1/T2/T3a/T3b/T4 post-fix tests on production all PASS, no duplicates, `ordine_ref` preserved on cancellation, DB cleaned post-test, `storico`/`archivio_conv`/`manual_giros`/`geo_cache` untouched. Residual non-blocking risks recorded in MASTER_CONTEXT and ROADMAP.

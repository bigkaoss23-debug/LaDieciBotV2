...
export function proposeForNewOrder(orders, newOrder, options = {}, deps = {}) {
  ...
  // ── Slot-search (28/05/2026, ex barriera-globale driverLiberoMin) ─────────
  ...
  // Regola prodotto: un ordine futuro (es. prenotazione 23:00) è un intervallo
  // locale, NON una barriera globale → non blocca più gli slot tra now e 23:00.
  ...
  if (horaNewMin != null) {
    ...
    if (partFeasible && slotFree) {
      // Hora richiesta fattibile e slot libero → passa così com'è.
      ok = true;
    ...
    // "Quando potete?" → primo buco reale a partire da now (non da fine coda).
    ok = true;

...
// STALE SERVICE PROTECTION V1 — the Finalizar servicio confirmation flow is
// now one shared component (also mounted by ServiceExceptionPanel for the
// PREVIOUS_SERVICE_PENDING recovery surface). Same scan/preflight/confirm,
// same copy, same data-testids.
...
const ServicioPage = ({onBack,onCloseout,ordenes,setOrdenes,waMsgs,setWaMsgs,notify,syncStatus,convConfermata=[],pendingPatches,onMesaPhoneShellActiveChange}) => {
  ...
  const [finalizarOpen, setFinalizarOpen] = useState(false); // STALE SERVICE PROTECTION V1 — drives the shared FinalizarServicioModal
  ...
  return (
    <div className="ld-viewport-shell" style={{background:C.nero,overflow:"hidden",display:"flex",
      ...
      <div style={{
        ...
        <div className="svc-actions">
          <div className="svc-act-row">
            <button data-testid="servicio-finalizar-btn" onClick={()=>setFinalizarOpen(true)}
      ...
      {/* Finalizar servicio — the shared confirmation flow (scan + economic
          preflight + chiudiServizio). Identical markup, copy and data-testids
          to the inline modal it replaces; also mounted by ServiceExceptionPanel
          for the PREVIOUS_SERVICE_PENDING recovery surface. */}
      <FinalizarServicioModal
        open={finalizarOpen}
        onClose={()=>setFinalizarOpen(false)}
        onClosed={()=>{ setFinalizarOpen(false); if (onCloseout) onCloseout(); }}
        notify={notify}
      />
